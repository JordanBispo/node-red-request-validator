...
String.prototype.isPhoneNumber = function () {
    return /^\+?\d{1,4}?[-.\s]?\(?\d{1,3}?\)?[-.\s]?\d{1,4}[-.\s]?\d{1,4}[-.\s]?\d{1,9}$/.test(this);
}
...
const validateQueryString = (queryString, expectedType, specifications = {}) => {
    ...
    if (expectedType === 'number') {
        ...
        expectedType = specifications?.format || expectedType
    }
    return validateStringByFormat(queryString, expectedType, specifications?.max, specifications?.min)
...
const QueryValidator = (query, schema) => {
    ...
    for (let key of schemaKeys) {
        ...
        if (query.hasOwnProperty(key)) {
            ...
            const gen_string_length_message = (max, min) => {
                if (max && min)
                    if (max === min)
                        return `and must have '${max}' ${expected_type === 'string ' ? 'characters ' : ''}`
                    else
                        return `and must be between '${min}' and '${max}' ${expected_type === 'string ' ? 'characters ' : ''}`
                else if (max)
                    return `and must be less than '${max}' ${expected_type === 'string ' ? 'characters ' : ''}`
                else if (min)
                    return `and must be greater than '${min}' ${expected_type === 'string ' ? 'characters ' : ''}`
                else
                    return ''
            }

            if (!validateQueryString(query_string, expected_type, specifications)) {
                return `'${key}' must be a ${expected_type} ${specifications?.format ? `with format ${specifications?.format}, ` : ''}${gen_string_length_message(specifications?.max, specifications?.min)}in query string`
            }
